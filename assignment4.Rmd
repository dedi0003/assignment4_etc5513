...
melb_top100 <- melb %>% 
  select(-offers, -establishment_types, -all_reviews.reviews) %>% 
...
```{r maps, fig.cap="Best of Melbourne", out.width= "100%", cache= TRUE, echo = FALSE}
...
map_melb <- leaflet(melb_best) %>%
  ...
  addCircleMarkers(lng=melb_best$longitude, 
                   lat=melb_best$latitude, label = melb_best$name,
  ...
  setView(options = , zoom = 12)

map_melb
```

As we can see from figure \@ref(fig:maps) above, that most of the best restaurans in Melbourne are concentrated in Melbourne CBD area. There is only one restaurant which located outside the CBD, which is **Shira Nui** where located in Glen Waverley area.
The map also showas whether the restaurant is expensive or not which indicated by larger circle if the average price for two persons is above $50.
