...
* Exploring restaraunts with gluten free food in Melbourne
...
```{r cost-rating-relationship, fig.cap= "Average Cost for Two to Aggregate Rating Relationship", echo = FALSE}
...
\section*{Exploring gluten free food/restaurants in Melbourne}
...
To look for a good restaraunt we will be using aggregate ratings.
Popular restaraunts will have more then 500 reviews. 
...
For diet concious people and people with Gluten Allergy.
...
```{r good-restaraunt-gluten-free, fig.cap = "Top Options", echo = FALSE}
...
The Table \@ref(fig:good-restaraunt-gluten-free) shows the name of the restarunts in Melbourne, their locality, number of reviews recieved, aggregate ratings, photo count and how much it costs for one persons gluten free meal on average. 

## Visualizing findings!


```{r review-price-relation, fig.cap= "Worth the money!"}
...
plotly::ggplotly(gluten)
```

The graph Figure \@ref(fig:review-price-relation) shows top restaraunts in Melbourne, using the category "gluten free", with an intercept at 500 reviews count. In the highest rating of 4.9, Lune Croissanterie is the best restaraunt with very reasonable average price of 12.50 AUD per person. While, Vue de monde is second on the list with a rating of 4.8 but very pricy at an average cost of 300 AUD.


```{r instagrammable-plot, fig.cap = "Can I instagram it?"}
...
plotly::ggplotly(glutenf)
```

The graph Figure \@ref(fig:instagrammable-plot) visualises restaraunts with good reviews and potential of each restaraunt on how much instagrammable it is. Lune Croissanterie has the best review rating with a high photo count of 1813. While, The Hardware Societe has the highest photo count at 4224, with a good rating of 4.6.

In Table \@ref(tab:top10-Jap) we can look at the top 10 popular japanese restaurants in Melbourne alongwith their location and rating. 'Don-Don' is a very popular japanese restaurant with 3168 votes. 'Shira Nui'is the highest rated restaurant with an aggreated rating of 4.8 out of 5.
...
From this analysis we find there is no real relationship between restaurant prices and quality suggesting expensive food is not better than cheap. Most of the very best restaurants in Melbourne is located near CBD.
